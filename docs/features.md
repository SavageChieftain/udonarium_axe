...
- **ボタン 1 つで組み立てる** — 部屋と通路、またはセルオートマトンの洞窟を作り、地形（terrain）として新しいテーブルに置く。壁は視線と光を遮る地形なので、そのまま視界システムと噛み合う
- **雰囲気プリセット** — 石造迷宮 / 地下墓所 / 遺跡 / 洞窟 / 溶岩洞 / 氷窟 / 砂の墳墓。生成アルゴリズムと既定素材に加えて、暗闇の深さ・環境光・グリッド表示までプリセットが決める
- **素材** — 壁 12 種・床 30 種のスウォッチから選ぶ。画像保管庫の自前の画像も指定できる。雰囲気を切り替えると既定へ追随するが、一度手で選んだ素材はそのまま残る
...
- **部屋の役割** — 入口の間 / 大広間 / 宝物庫 / 最奥 / 行き止まり / 小部屋を、部屋をつないだ最小全域木の形から割り当てる
- **GM 用の概要** — 部屋番号・寸法・接続・施錠・鍵の在り処・松明の位置を、生成パネルの中に一覧で出す（コピーできる）。共有メモは卓の子ではなく全卓に残ってしまうため、盤の上には置かない。未探索を隠すスクラッチマスクは卓の子なので盤面いっぱいに敷く
- **松明** — 暗闇を有効にするプリセットでは、部屋の中の壁際に 1 マスの光る地形を最大 5 基置く。壁ブロックそのものを光らせると、その地形が光を遮らなくなって最大 12 マス四方の穴が開く。かといって光源オブジェクトは卓の子ではないので、卓を消しても残ってしまう。1 マスの地形ならどちらも避けられる
- **生成前のプレビュー** — レイアウトを SVG で描き、置かれる地形の個数と同期データ量を出す。150 個で警告、300 個で生成を止める
- **卓は自動で切り替わらない** — セッション中に用意してもプレイヤーが飛ばされない。移動はパネルのボタンから。振り直すと前回の卓を破棄する。生成するものはすべて卓の子なので、卓を消せば残らない
- **正方グリッド固定** — ヘクスでは地形がヘクスリングに切り抜かれて矩形へのマージが効かないため、生成するテーブルは正方グリッドにしている
- **通路の経路** — 部屋の中心どうしを最短経路でつなぐが、つなぐ相手でない部屋の脇を通るセルにコストを乗せてある。壁に密着して並走されると、そのあいだに石が無いぶん部屋の側面が丸ごと開口になり、通路が部屋を貫通したように見えるため。遺跡プリセットの崩落も、部屋に接する石は崩さない
- **矩形マージ** — 隣り合うセルをまとめて 1 つの地形にする。40×30・部屋 8 個で 1,200 個が 69 個に減り、同期もマス目ごとの視線計算も軽くなる
