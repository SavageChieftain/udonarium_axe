...
- **松明** — 暗闇を有効にするプリセットでは、部屋の中の壁際に最大 5 基の光源を立てる。壁の地形そのものを光らせると、その地形が光を遮らなくなって最大 12 マス四方の穴が開くため、光源は独立したオブジェクトにしている
- **生成前のプレビュー** — レイアウトを SVG で描き、置かれる地形の個数と同期データ量を出す。150 個で警告、300 個で生成を止める
- **卓は自動で切り替わらない** — セッション中に用意してもプレイヤーが飛ばされない。移動はパネルのボタンから。振り直すと前回の卓を破棄する
