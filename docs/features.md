...
- **招待リンク** — 接続パネルで部屋 ID / 部屋名 / 合言葉（任意）/ ロールを載せたリンクを生成。開くと起動時に自動入室し、進行状況と失敗時のロビー導線をバナーで表示。リンクはハッシュフラグメントに載せるためサーバーログに残らない（`domain/peer/invite-link`、`features/lobby/invite-join`）
