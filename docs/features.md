...
- **スキン** — 光源のコンテキストメニューから絵柄を選ぶ。同梱は焚き火・壁掛け松明・火鉢・ランタンの 4 種で、画像保管庫の自前の画像も指定できる。スキンなしなら従来の電球アイコン

## ダンジョン生成

- **ボタン 1 つで組み立てる** — 部屋と迷路、またはセルオートマトンの洞窟を作り、地形（terrain）として新しいテーブルに置く。壁は視線と光を遮る地形なので、そのまま視界システムと噛み合う
...
- **GM 用の概要** — 部屋番号・寸法・接続・施錠・鍵の在り処・松明の位置を、生成パネルの中に一覧で出す（コピーできる）。共有メモは卓の子ではなく全卓に残ってしまうため、盤の上には置かない。未探索を隠すマップマスクを盤面いっぱいに敷く。削れるのはマップマスクの方で、名前の似たスクラッチマスクは動かすか消すかしかできない一枚布。マスクは壁より高い位置に置く（床の高さに置くと地形の下に潜って何も隠せない）
- **明かり** — 暗闇を有効にするプリセットでは、1 マスの光る地形を最大 5 基置く。狭い部屋には壁掛け松明（壁に接する床に置き、石から離れる向きへ 200 度の光を投げる）、広い部屋には焚き火か火鉢（周りに立てる空きのある床へ）。壁ブロックそのものを光らせると、その地形が光を遮らなくなって最大 12 マス四方の穴が開くため、明かりは必ず 1 マスの独立した地形にする
- **生成前のプレビュー** — レイアウトを SVG で描き、置かれる地形の個数と同期データ量を出す。200 個で警告、400 個で生成を止める
- **卓は自動で切り替わらない** — セッション中に用意してもプレイヤーが飛ばされない。移動はパネルのボタンから。振り直すと前回の卓を破棄する。生成するもの（地形・扉・明かり・マスク）はすべて卓の子なので、卓を消せば残らない
