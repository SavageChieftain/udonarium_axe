...
- **扉** — 視線を遮る地形として実体化する。入口から最奥へ至る経路の最後の 1 枚を施錠し、鍵は経路の外にある部屋へ置く。分岐を探索しなければ奥へ進めない
- **部屋の役割** — 入口の間 / 大広間 / 宝物庫 / 最奥 / 行き止まり / 小部屋を、部屋をつないだ最小全域木の形から割り当てる
- **GM 用の下拵え** — 部屋番号・接続・鍵の在り処を書いた概要メモと、各部屋に置く番号メモ（どちらも GM 限定）。未探索を隠すスクラッチマスクを盤面いっぱいに敷く
- **松明** — 暗闇を有効にするプリセットでは、部屋に面した壁に最大 6 基の光源を灯す。光源 1 つごとに影の計算が走るため上限を設けている
