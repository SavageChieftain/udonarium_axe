...
- **見学（読み取り専用）** — 卓上の操作・編集・作成を一切ブロック
- **GM の可視性** — 隠しキャラ・シークレットダイス／チャット・隠し名の閲覧
- **GM 不在の担保** — リロード／再接続でロールを保持、誰でも GM を引き継ぎ可能
- **公開範囲（ディスクロージャ）** — キャラ・共有メモ・カード・ダイス単位で「全員／特定PL／GMのみ」を設定（→ [情報の公開範囲](/manual/disclosure)）
- **オーナー** — オブジェクトの持ち主を割り当て、オーナー本人も自分の公開範囲を設定可能
- **非公開時の見え方** — 存在は残しつつ詳細・プレビュー・リモコン等を遮断、共有メモは本文をモザイク表示
- **チャットタブ権限** — タブごとに PL／見学の **閲覧・発言**を個別設定。閲覧不可タブは非表示、発言不可タブは入力欄を閲覧専用化。ログ書き出しも権限に追従（→ [チャット](/manual/chat)）
