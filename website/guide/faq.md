# よくある質問

導入と運用でつまずきやすいところを、質問の形でまとめています。

## スマートフォンで遊べますか？

はい。幅の狭い画面では[専用のレイアウト](/manual/mobile)に切り替わり、卓とチャットの上下分割・下部のメニュー・
長押しのコンテキストメニューで操作できます。横向きでも、ホーム画面に追加してアプリのように起動しても使えます。

動作確認がもっとも厚いのは **デスクトップ版 Chrome** です。マップエディターのような描画中心の機能は、
指よりマウスのほうが精密に扱えます。

## 他のツールで作ったキャラクターを取り込めますか？

はい。**ココフォリアのコマ JSON**、**キャラクター保管所**・**キャラクターシート倉庫**の JSON / URL、
**CharaXiv** の「ココフォリア用コピー」出力から、AXE のキャラコマを生成できます。
画面左の FAB メニュー →「キャラ取り込み」から貼り付けてください。
詳しくは [キャラクターの取り込み](/manual/character-import) を参照してください。

## ココフォリアの卓をそのまま持ってこられますか？

**実験的機能として対応しています。** ココフォリアで書き出したルームデータ zip をテーブルに落とすと、
盤面画像・マス数・画像パネル・コマを再現します。**シーンはテーブル 1 枚ずつ**に写るので、
テーブルの切り替えがそのままシーン切り替えになります。
ただし **BGM はココフォリアの仕様上 zip に含まれず**、カード・カットインも未対応です。
詳しくは [ココフォリアのルーム取り込み](/manual/room-import) を参照してください。

## 音は鳴るのに、演出が画面に出ません

まず疑うのは OS 側の「動きを減らす」設定です。AXE は既定でこれに従い、演出の絵だけを止めて効果音は残します。

Windows 11 なら **設定 → アクセシビリティ → 視覚効果 → アニメーション効果** を確認してください。
この項目は描画を軽くする目的でも切られるため、意図せず入っていることがあります。ブラウザを変えても
同じように出ないなら、まずこれを疑ってください。OS の設定なので、Chrome も Firefox も Edge も同じ値を読みます。

OS の設定はそのままにして卓の演出だけ出したいときは、FAB メニューの
[エフェクトボタン](/manual/theme#エフェクトの表示)を **常に再生** にしてください。

雨が降らずに画面が少し暗くなるだけ、霧が白くなるだけ、というのも同じ症状です。空気の色は止めても残り、
降る粒だけが消えるためこう見えます。

## なぜバックエンドが必要なのですか？

P2P 接続を確立するには SkyWay の認証トークンが要ります。発行には App ID / Secret を使った署名が必要で、
Secret をブラウザに置くわけにはいきません。そのため、**トークンを発行する小さなバックエンドを 1 つ**用意します。
詳しくは [必要なもの](/guide/requirements) を参照してください。

## ゲームデータはサーバーに保存されますか？

いいえ。テーブル上のオブジェクトは WebRTC（SkyWay SDK v2）の P2P 通信でブラウザ間に直接同期され、
**ゲームデータが中央サーバーに保存されることはありません**。バックエンドはトークン発行のみを担います。

## 本家 Udonarium のバックエンドは使えますか？

はい。本家 [TK11235/udonarium-backend](https://github.com/TK11235/udonarium-backend) も**そのまま利用できます**。
Axe が呼び出す API（`GET /v1/status`・`POST /v1/skyway2023/token`）と完全に同一の仕様です。
選択肢の一覧は [バックエンドの選択肢](/guide/backend) にまとめています。

## どのバックエンドを選べばよいですか？

- とにかく手軽に始めたい → **Vercel Edge**（[udonarium-backend-vercel](https://github.com/Xelltis/udonarium-backend-vercel)）
- レンタルサーバーを持っている → **PHP 8.3 / Apache**（[udonarium_axe_backend](https://github.com/Xelltis/udonarium_axe_backend)）
- Cloudflare Workers / AWS Lambda / 自前 Node で運用したい → **本家バックエンド**

## 接続できないときは？

1. `https://<バックエンドのURL>/v1/status` を開いて `OK` が返るか確認する
2. バックエンドの `ACCESS_CONTROL_ALLOW_ORIGIN` が、Axe を公開している URL を許可しているか確認する
3. フロントエンドの `assets/config.json` の `backend.url` が正しいバックエンド URL を指しているか確認する

それでも解決しない場合は [GitHub の Issue](https://github.com/Xelltis/udonarium_axe/issues) で報告してください。
