...
導入と運用でつまずきやすいところを、質問の形でまとめています。

## スマートフォンで遊べますか？

はい。幅の狭い画面では[専用のレイアウト](/manual/mobile)に切り替わり、卓とチャットの上下分割・下部のメニュー・
長押しのコンテキストメニューで操作できます。横向きでも、ホーム画面に追加してアプリのように起動しても使えます。

動作確認がもっとも厚いのは **デスクトップ版 Chrome** です。マップエディターのような描画中心の機能は、
指よりマウスのほうが精密に扱えます。
...
## ココフォリアの卓をそのまま持ってこられますか？

**実験的機能として対応しています。** ココフォリアで書き出したルームデータ zip をテーブルに落とすと、
盤面画像・マス数・画像パネル・コマを再現します。**シーンはテーブル 1 枚ずつ**に写るので、
テーブルの切り替えがそのままシーン切り替えになります。
ただし **BGM はココフォリアの仕様上 zip に含まれず**、カード・カットインも未対応です。
詳しくは [ココフォリアのルーム取り込み](/manual/room-import) を参照してください。

## なぜバックエンドが必要なのですか？

P2P 接続を確立するには SkyWay の認証トークンが要ります。発行には App ID / Secret を使った署名が必要で、
Secret をブラウザに置くわけにはいきません。そのため、**トークンを発行する小さなバックエンドを 1 つ**用意します。
