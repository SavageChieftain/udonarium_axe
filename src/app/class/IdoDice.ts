...
export default class IdoDice extends Base {
  ...
  static readonly COMMAND_PATTERN =
    /^S?([+\-(]*\d+|\d+B\d+|C[+\-(]*\d+|choice|D66|(repeat|rep|x)\d+|\d+R\d+|\d+U\d+|BCDiceVersion|ID\d*<=[+\-(]*\d+|RES\([+\-\d()/*CR]+\)|CBR\(\d+,\d+\))/i;
...
`.trim();
  ...
  constructor(command: string, internal?: BaseInstance) {
    ...
    if (matchResult) {
      if (matchResult[3].split(/\s/)[0] == '') {
        // マッチ部分以降が空であるかスペース区切りのコメントかチェック
        console.log('matchResult' + matchResult);
  ...
  private optionCommand(command: string): void {
    this.critical = 5;
    this.famble = 96;
    this.special = 5;
  ...
  private custom_dice_d100(): Result | null {
    ...
    let newResultText = '';
    for (let i = 0; i < this.repeat; i++) {
      const dice = result.detailedRands[i].value;
      // 結果のテキストを作成
      if (this.repeat > 1) {
        if (i > 0) {
          newResultText += '\n\n';
