...
describe('buildCardContextMenu()', () => {
  it('isLock=false なら「固定する」「固定マーク」関連は出ない', () => {
    ...
    const menu = buildCardContextMenu(card as unknown as Card, 50, {
      onCreateStack: vi.fn(),
      onShowDetail: vi.fn(),
    });
  ...
  it('isVisible=true && !isHand で「裏にする」、isVisible=false で「表にする」が出る', () => {
    const visible = buildCardContextMenu(makeCard({ isVisible: true, isHand: false }) as unknown as Card, 50, {
      onCreateStack: vi.fn(),
      onShowDetail: vi.fn(),
    });
    expect(names(visible)).toContain('裏にする');

    const hidden = buildCardContextMenu(makeCard({ isVisible: false, isHand: false }) as unknown as Card, 50, {
      onCreateStack: vi.fn(),
      onShowDetail: vi.fn(),
    });
  ...
  it('「重なったカードで山札を作る」が onCreateStack を呼ぶ', () => {
    ...
    const menu = buildCardContextMenu(makeCard() as unknown as Card, 50, {
      onCreateStack,
      onShowDetail: vi.fn(),
    });
