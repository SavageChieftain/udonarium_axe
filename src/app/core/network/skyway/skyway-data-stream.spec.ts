...
describe('SkyWayDataStream receivedMap クリーンアップ', () => {
  it('dispose 時に receivedMap がクリアされる', () => {
    const receivedMap = new Map<string, { id: string; chunks: Uint8Array[]; length: number; byteLength: number; createdAt: number }>();
  ...
  it('TTL 超過チャンクが evictStaleChunks で削除される', () => {
    ...
    receivedMap.set('old-chunk', { id: 'old-chunk', chunks: [], length: 0, byteLength: 0, createdAt: now - CHUNK_TTL_MS - 1 });
